...
import FilterItem from "../../Atoms/Filter";
...
function Filters() {
	...
	return (
		<div className="dropdown dropdown-left dropdown-end absolute bottom-16 right-0 z-30">
			...
			<ul
				tabIndex={0}
				className="card p-1 mr-2 text-gray-500 dropdown-content shadow bg-base-100 rounded w-32"
			>
				{filters.map((filter, idx) => {
					...
					return (
						<FilterItem
							text={TFilter}
							key={idx + filter}
							setFilter={() => dispatch(setFilter(TFilter))}
						/>
					);
				})}
			</ul>
